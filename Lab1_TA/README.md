...
To laboratorium pokazuje podstawowy proces oceny aplikacji działających na tradycyjnym serwerze aplikacyjnym WebSphere pod kątem ich modernizacji na środowisko kontenerowe i Liberty. Dowiesz się również, jakie wartości wynikają z korzystania z narzędzia Transformation Advisor (TA) do oceny lokalnych aplikacji Java i zidentyfikowania odpowienich kandydatów do modernizacji aplikacji.

Po ukończeniu tego laboratorium zdobędziesz doświadczenie w korzystaniu z TA w celu szybkiej analizy lokalnych aplikacji Java (bez wglądu do ich kodu źródłowego), szacowania pracy związanej z modernizacją operacyjną i korzystania z akceleratorów wsparcia wdrożenia TA w celu przyspieszenia uruchomienia aplikacji na Liberty i w kontenerach.

**IBM Cloud Transformation Advisor** (Transformation Advisor) to narzędzie wspierające proces modernizacji aplikacji, które jest dostępne w ramach licencji IBM Cloud Pak for Applications i IBM WebSphere Hybrid Edition. 

**Narzędzie Transformation Advisor zapewnia następującą warości:**
  - ocena kosztów i złożoność pracy pod kątem modernizacji aplikacji na środowisko kontenerowe i WebSphere Liberty.
...
Ponadto narzędzie zapewnia rekomendację odpowiedniej edycji IBM WebSphere Application Server i oferuje porady, najlepsze praktyki oraz potencjalne rozwiązania wpierające modernizacje aplikacji do Liberty lub środowisk kontenerowych. Przyspiesza proces migracji aplikacji do chmury, minimalizuje błędy i ryzyko oraz skraca czas wprowadzania na rynek.
...
Po określeniu aplikacji do modernizacji do WebSphere Liberty, wykorzystasz akceleratory wygenerowane przez TA do wdrożenia i uruchomienia aplikacji na WebSphere Liberty na lokalnym komputerze dewelopera w kontenerze, aby sprawdzić poprawność rozwiązania.

## 2. Cele

Cele tego laboratorium:
...
Aby określić, jakie modele programowania Java EE wykorzystują aplikacje znajdujące się na serwerze, zwykle uruchamia się narzędzie Transformation Advisor **Data Collector** na analizowanym serwerze WebSphere.
...
W kolejnym kroku wykorzystasz narzędzie Transformation Advisor do przeanalizowania tych aplikacji w celu zidentyfikowania dobrego kandydata do przeniesienia do Liberty.
___

### 6.2 Uruchomienie Transformation Advisor (lokalnie)

Transformation Advisor może oceniać dowolne aplikacje oparte na Javie. W tym laboratorium użyjesz TA do oceny, czy lokalna aplikacja WebSphere - **Mod Resorts**, nadaje się do przeniesienia do Liberty i co należy zrobić, aby wdrożyć ją na środowisku kontenerowym.
...
5.  Aplikacja **Transformation Advisor** została wystartowana, kliknij prawym przyciskiem myszy łącze URL aplikacji i wybierz **`Open Link`**, aby uruchomić ją w oknie przeglądarki internetowej. 
    ...
    W następnej sekcji użyjesz interfejsu użytkownika Transformation Advisor, aby pobrać **narzędzie do zbierania danych - Data Collector** dla środowiska, w którym będzie działał serwer aplikacji WebSphere, takiego jak Windows lub Linux.

### 6.3 Pobierz narzędzie Transformation Advisor Data Collector

Po uruchomieniu Transformation Advisor należy pobrać narzędzie Data Collector, które będzie analizowało serwer WebSphere Application Server.

Aby ocenić lokalne aplikacje Java, należy uruchomić narzędzie **Transformation Advisor Data Collector** dla środowiska serwera aplikacji. Wyodrębni ono informacje o aplikacjach uruchomionych na środowisku WAS. Narzędzie można pobrać z interfejsu użytkownika Transformation Advisor.
...
1.  Na stronie głównej Transformation Advisor utwórz **new workspace**.
    ...
    Po utworzeniu obszaru roboczego dostępne będzią opcje **Download** narzędzia Data Collector oraz **Upload** istniejącego pliku danych.
...
3.  Na stronie pobierania można pobrać odpowiednią wersję narzędzia Data Collector w zalenżności od źródłowego systemu operacyjnego, gdzie działa analizowany serwer aplikacyjny WebSphere. Znajdziesz tam informacje o sposobie wykorzystania narzędzia z poziomu wiersza poleceń, do zbierania danych aplikacji i konfiguracji z serwerów WebSphere, WebLogic i Tomcat.
    
    a.  Ponieważ laboratoryjna maszyna wirtualna to system operacyjny Linux, kliknij **`Download for Linux`**, aby pobrać narzędzie.

    ![A screenshot of a social media post Description automatically generated](./images/media/image16.png)

    Spakowany plik narzędzia Data Collector zostanie zapisany w katalogu **/home/techzone/Downloads** maszyny wirtualnej **Stacji roboczej**.
...
### 6.4 Symulacja uruchomienia narzędzia Transformation Advisor Data Collector

Po pobraniu narzędzia Data Collector należy je rozpakować i uruchomić dla serwserwera WebSphere Application Server (WAS) w celu zebrania wszystkich danych wdrożonych aplikacji i ich konfiguracji z serwera WAS.

Teraz `zasymulujmy` kroki, aby uruchomić Data Collector.

1.  Wróć do okna `Terminal` i przejdź do katalogu **/home/techzone/Downloads**, a następnie wyświetl jego zawartość za pomocą poleceń:
...
2.  Wyodrębnij narzędzie Data Collector za pomocą następującego polecenia:
    ...
    **Uwaga:** W tym momencie Data Collector jest gotowy do uruchomienia.
    ___


3.  Wróć do interfejsu użytkownika Transformation Advisor w przeglądarce internetowej, aby wyświetlić sekcję „**Run the Tool**”, która pokazuje polecenie do uruchomienia dla środowiska WebSphere.
    
    a.  Na stronie **Data Collector** przewiń w dół do sekcji „**Run Tool**”.

    Polecenie Data Collector, które zostanie wykonane, jest oparte na **domenie** i **typie analizy** wybranych w tej sekcji.
 
    ![](./images/media/image19.png)

4.  Wybierz domenę **IBM WebSphere**. Inne domeny obejmują inne środowiska wykonawcze Java EE.
...
5.  Wybierz typ analizy „**`Apps and Configuration`**”.
    ...
    Wybranie **Apps & Configuration** gwarantuje, że dane aplikacji i dane konfiguracyjne serwera będą zebrane.
...
2.  Wpisz **1**, aby zaakceptować umowę licencyjną i naciśnij **Enter**, jak pokazano poniżej. 
    ...
    **Tylko w celach informacyjnych, NIE WYKONUJ poleceń**
    ...
    Ogólnie rzecz biorąc, jeśli serwer aplikacji i **Transformation Advisor** znajdują się w tej samej infrastrukturze sieciowej, zebrane dane zostaną automatycznie przesłane do **Transformation Advisor**. W przeciwnym razie należy ręcznie przesłać dane do **Transformation Advisor**.

### 6.5 Załadowanie zebranych danych do Transformation Advisor

W tej sekcji załadujesz plik zip kolekcji danych „**AppSrv01.zip**” ze środowiska WebSphere do analizy.

___

**Uwaga:** Jak wspomniano w poprzedniej sekcji, kroki zbierania danych zostały już wykonane, a wynikowy plik **`AppSrv01-3.8.1.zip`** jest dostępny w środowisku laboratoryjnym.
___
...
2.  Kliknij ikonę **`Collections`** i wyświetl nazwę kolekcji: „**admin.ibm.demo**”.
    ...
    Całkowity koszt to liczba dni potrzebnych na modernizacje (modyfikacje w kodzie i konfiguracji) aplikacji w celu uruchomienia na wybranym **środowisku docelowym**. W tym przykładzie **WebSphere Liberty** jest wybranym celem migracji.
 
    **Celem migracji mogą być:**
    ...
    Można wybrać wiele środowisk docelowych, a Transformation Advisor przedstawi całkowity koszt obszaru roboczego dla każdego z wybranych celów.
...
3.  W sekcji **Migration Targets** wybierz wszystkie **trzy** cele migracji i zwróć uwagę na różne koszty modernizacji związane z migracją wszystkich 5 aplikacji w obszarze roboczym do różnych docelowych środowisk migracji.
    ...
    - **Migration plan**: pliki wspomagające wygenerowane przez Transformation Advisor, aby pomóc w budowaniu i wdrażaniu wybranej aplikacji w docelowym środowisku uruchomieniowym.
...
Dowiesz się, dlaczego różne środowiska docelowe mają różne koszty developmentu i potencjalne problemy. 

1.	Na stronie „**Wszystkie aplikacje Java**” przefiltruj **CustomerOrderServicesApp.ear** dla wszystkich trzech celów migracji, używając pola wyszukiwania „**Customer**”.
...
2.	Pierwszą rzeczą, którą rzuca się w oczy to liczba problemów i koszt developmentu związany z aplikacją dla każdego z trzech środowisk docelowych:
    ...
    - Przeniesienie aplikacji do **Open Liberty** ma poziom złożoności **MODERTATE**. Wymagana jest zmiana kodu, a część zmian kodu można zautomatyzować. Wymaga 3,5 dnia developmentu, aby rozwiązać 1 poważne, 2 niewielkie problemy i 4 kwestie informacyjne.
...
Następnie zbadajmy, dlaczego koszty developmentu i liczba problemów są wyższe w przypadku przejścia na Open Liberty w porównaniu do WebSphere Liberty. 
...
1.	Przejrzyj szczegóły dotyczące CustomerOrderSrvicesApp.ear dla **WebSphere Liberty**.
    ...
    *W tym przykładzie zgłoszono 3 kwestie informacyjne:*
    ...
    c.	Przewiń stronę do dolnej części szczegółów aplikacji, aby znaleźć dostępne raporty. 
...
Spójrzmy, dlaczego przeniesienie aplikacji na platformę Open Liberty wymaga więcej pracy, niż przeniesienie na platformę WebSphere Liberty. 
...
Transformation Advisor nie tylko dostarcza przydatnych informacji na temat aplikacji, które rozważasz zmodernizować do WebSphere Liberty lub Open Liberty, ale także generuje akceleratory do budowania i wdrażania aplikacji na Liberty, kontenerach i na Kubernetes. 

W tej sekcji przyjrzymy się **konfiguracji serwera Liberty** `server.xml`, którą TA generuje na podstawie analizy konfiguracji WebSphere, gdy Transformation Advisor Data Collector został uruchomiony dla serwera WebSphere na maszynie wirtualnej.  
...
7.	Przejrzyj zawartość pliku **server.xml**.
    ...
    Transformation Advisors Data Collector przeanalizował aplikacje i konfigurację serwera WebSphere. Dane konfiguracyjne serwera zostały wykorzystane do wygenerowania odpowiedniego pliku server.xml w celu skonfigurowania aplikacji na serwerze Liberty. 

    a.	Funkcje **Liberty** używane przez aplikację są skonfigurowane. 
...
3.	W polu **"search"** wpisz **`modresorts`**, co zawęzi listę aplikacji tylko do aplikacji Mod Resorts dla wszystkich trzech celów migracji. 
    ...
    Należy zauważyć, że aplikacja `modresorts` jest również łatwa do migracji do WebSphere Liberty i WebSphere traditional. 
    ...
    Przyjrzymy się szczegółowo wynikom analizy aplikacji **moderesorts-1.0_war**.ear.
...
8. Wróć do strony Transformation Advisor i kliknij łącze **`Inventory Report`**.
    ...
    a.  Przewiń w dół `Inventory Report`, aby przeanalizować ten raport, informujący o tym, co znajduje się w środowisku uruchomieniowym aplikacji i pomagający lepiej zrozumieć środowisko uruchomieniowe aplikacji, jego komponenty i relacje między nimi.
...
Ze spostrzeżeń uzyskanych z Transformation Advisor dowiedziałeś się, że Mod Resorts jest prostą aplikacją Java i jest dobrym kandydatem do przejścia na Open Liberty i kontenery.
...
4.  Kliknij łącze **`Download`**, aby pobrać pakiet artefaktów do lokalnego systemu plików na maszynie wirtualnej stacji roboczej.
...
6.  Wyświetl plik **server.xml**. NIE NALEŻY ZMIENIAĆ ZAWARTOŚCI PLIKU.
...
7. **Zamknij** edytor po zakończeniu przeglądania pliku. **NIE ZAPISUJ żadnych zmian** w jego zawartości.


8. Wyświetl plik **Containerfile**. Nie zmieniaj zawartości pliku.
    ...
    - Plik Containerfile podzielony jest na dwa etapy budowania obrazu kontenera dla aplikacji Mod Resorts.
...
9. **Zamknij** edytor po zakończeniu przeglądania pliku. **NIE ZAPISUJ** żadnych zmian w jego zawartości.
    
10. Na koniec należy dodać plik wykonywalny aplikacji (.war) Mod Resorts do katalogu docelowego pakietu migracyjnego.
...
1.  Zbuduj obraz Docker, który zawiera aplikację modresorts na Open Liberty
    ...
    - Obraz kontenera jest tworzony na podstawie dyrektyw w pliku **Containerfile** określonym przez parametr -f.
    
    - Obraz kontenera jest budowany przy użyciu artefaktów w bieżącym katalogu pakietu migracyjnego
...
5.  Uruchom aplikację modresorts z poziomu przeglądarki internetowej.
    ...
    a.  Możesz kliknąć menu rozwijane „**Where to?**” i wybrać miejsce docelowe
